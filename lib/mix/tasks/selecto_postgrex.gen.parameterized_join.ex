defmodule Mix.Tasks.SelectoPostgrex.Gen.ParameterizedJoin do
  ...
  ## Examples
      ...
      mix selecto_postgrex.gen.parameterized_join products category:string active:boolean

      # Generate with field specifications
      mix selecto_postgrex.gen.parameterized_join products category:string,required active:boolean,default=true --fields name:string,price:decimal

      # Generate with join condition template
      mix selecto_postgrex.gen.parameterized_join discounts category:string --condition "discounts.category = :category AND discounts.active = :active"
  ...
  def run(args) do
    ...
    if join_name == nil or parameters == [] do
      Mix.shell().error("""
      Usage: mix selecto_postgrex.gen.parameterized_join JOIN_NAME PARAM1:TYPE PARAM2:TYPE [options]

      Examples:
        mix selecto_postgrex.gen.parameterized_join products category:string active:boolean
        mix selecto_postgrex.gen.parameterized_join discounts type:string amount:float,required --fields discount_percent:float
      """)
