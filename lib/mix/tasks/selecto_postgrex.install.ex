defmodule Mix.Tasks.SelectoPostgrex.Install do
  ...
  @moduledoc """
  Install Selecto ecosystem dependencies for Postgrex-based projects and run
  SelectoComponents asset integration.

  ## Examples

      mix selecto_postgrex.install
      mix selecto_postgrex.install --postgis
      mix selecto_postgrex.install --development-mode --source my-github-user
      mix selecto_postgrex.install --check
  ...
  def info(_argv, _composing_task) do
    %Igniter.Mix.Task.Info{
      group: :selecto,
      example: "mix selecto_postgrex.install --development-mode --source my-fork",
      schema: [
        postgis: :boolean,
  ...
  def igniter(igniter) do
    ...
    if check? do
      Mix.Task.run("selecto_postgrex.components.integrate", ["--check"])
      Igniter.add_notice(igniter, "Check complete. No files were modified.")
    else
      Mix.Task.run("selecto_postgrex.components.integrate", [])

      Igniter.add_notice(
        igniter,
        """
        Selecto Postgrex install complete.
    ...
    with {:ok, content} <- File.read(mix_file),
      ...
      if check? do
        if updated == content do
          ...
          Mix.shell().info("mix.exs would be updated with Selecto Postgrex dependencies")
        end
  ...
  defp dep_line_matches_apps?(line, apps) do
    Enum.any?(apps, fn app -> String.contains?(line, "{:#{app},") end)
  end
